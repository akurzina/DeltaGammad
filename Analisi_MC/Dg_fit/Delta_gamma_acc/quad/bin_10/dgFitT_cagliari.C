...
std::vector<float> hbins8 = {0.0005*1000,0.0007*1000,0.0009*1000,0.0011*1000,0.0013*1000,0.0015*1000,0.0017*1000,0.0019*1000,0.0021*1000,0.0024*1000,0.0028*1000,0.0035*1000,0.0050*1000,0.014*1000};
 ...
 */
 ...
 // h1 settings
  ...
  h8Kstar->SetMinimum(5000);  // Define Y ..
...
ValueWithError weightedAvg(std::vector<ValueWithError>& vals){
  ...
  std::vector<std::string> accslinear = {"acc_lin.root"};
  std::vector<std::string> accsquad = {"new_2016-quad.root"};
  std::vector<std::string> accsparab = {"acc_parab_hist.root"};
...
// run4(files, Ksdefaults, " ", "hdiv", 0.6583);  // for binned approach //
 ...
 // run4(files, Ksdefaults, "linear", "acclinear", 0.6583);
run4(files, Ksdefaults, "parab", "parab_acc", 0.6583);
