...
TH1F* makeHisto16(std::string name, std::string name2, int j){
...
TH1F* div = (TH1F*)h8Kstar->Clone("div");
 ...
 TH1F* res_histo = (TH1F*)file->Get(name_hist);

 TFile* file2 = new TFile(name2.c_str());
 TH1F* res_histo2 = (TH1F*)file2->Get(name_hist);
...
Result runFits(std::string Kstarname,std::string Ksname, std::string name = "2016-linear.root",std::string dset= "2016", bool mplot = false , std::string hname = "h8_bc",double gammaval =  0.6583, int j = 6){
  ...
  std::cout << "runFits before makeHisto16 "  << std::endl;
  TH1F* h;
  if (dset == "2016") h= makeHisto16(Kstarname,Ksname, j);
...
void run4(std::vector<std::string> files = defaultFiles, std::vector<std::string> Ksfiles = Ksdefaults,std::string accform = "NONE", std::string hname = "h8_bc",double gammaval =  0.6583, int n_toys = 6){

  std::vector<std::string> datasets ={"2016"};
  std::vector<std::string> accs;
  if (accform == "quad"){
    accs.push_back("accsquad");
  }
  else if (accform == "parab"){
    accs.push_back("accsparab");
  }
  else if (accform == "NONE"){
    accs.push_back("accsNONE");
  }
  else {
    accs.push_back("accslinear");
  }

  //std::cout << "run4: "  << files[1] << " and " << Ksfiles[1] << " Acceptance: " << accs[1] <<  std::endl;
  /*
  const int nbins_dg=18;
  Double_t  xbins_dg[nbins_dg]={-1e-7, -5e-8, -1e-8, -6e-9, -3e-9, -1e-9, -6e-10, -3e-10, -1e-11, 1e-11, 3e-10, 6e-10, 1e-9, 3e-9, 6e-9, 1e-8, 5e-8,1e-7};  
  */
  
  
  //TH1F* dg_hist = new TH1F("dg_hist","dg_hist", 20, -1e-7, 1e-7 );
  TH1F* dg_hist = new TH1F("dg_hist","dg_hist", 80, -1e-1, 1e-1 );
  TH1F* dg_sigma = new TH1F("dg_sigma","dg_hist", 80, 0.01, 0.03 );
  TH1F* pull_hist = new TH1F("pull","pull", 80, -7, 7);
  //TH1F* dg_hist = new TH1F("dg_hist","dg_hist", nbins_dg-1, xbins_dg);
  
  std::vector<ValueWithError> vals;
  for (int i = 0; i<n_toys; ++i){
    std::cout << "run4 cycle: "  << i << std::endl;
    Result res = runFits(files[0],Ksfiles[0],accs[0],datasets[0], true, hname,gammaval, i);
    vals.push_back(res.dg);
    double pull_value = vals[i].value/vals[i].error;
    dg_hist->Fill(vals[i].value);
    dg_sigma->Fill(vals[i].error);
    pull_hist->Fill(pull_value);
  ...
  TFile* file_dg= new TFile("dg_histo.root", "RECREATE");
  dg_hist->Write();
  file_dg->Close();
  
  TFile* file_dg_sigma= new TFile("dg_sigma_hist.root", "RECREATE");
  dg_sigma->Write();
  file_dg_sigma->Close();
  
  TFile* file_pull= new TFile("dg_pull.root", "RECREATE");
  pull_hist->Write();
  file_pull->Close();
  ...
  gROOT->Clear(); 
  cout.precision(8);
  TCanvas *c_dg = new TCanvas("c_dg", "canvas", 850,600);
  dg_hist->DrawNormalized();
  
 /* 
  for(int i = 0; i<vals.size(); i++){
      cout<<"PROVA STAMPA "<<vals[i].value<<"\n";
  }
 */
}
...
void altfits(){
   ...
   //file Jpsi Kstar
   std::vector<std::string> files = {"res_histo_star_toy.root"};
   //file Jpsi Kshort
   std::vector<std::string> Ksdefaults = {"res_histo_short_toy.root"};
   int n_toys = 10000;
   //std::cout << "Altfits: "  << files[1] << " and " << Ksdefaults[1] << std::endl;

   // run4(files, Ksdefaults, " ", "hdiv", 0.6583);  // for binned approach //
      run4(files, Ksdefaults, "NONE", "h8_bc", 0.6583, n_toys); // no acceptance h8_bc= name acceptance
