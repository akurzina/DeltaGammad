...
void risoluzione_short(){
        ...
        TChain *datatree_short_rec = new TChain("DecayTree");
	datatree_short_rec->Add("/home/akurzina/Analisi/Selezione_jpsikshort/Bd2JpsiKshort_MC_selected.root");
	  
        datatree_short_rec->SetBranchStatus("*",0);
	datatree_short_rec->SetBranchStatus("time", 1);
       
        TH1D *hmr_short=new TH1D("Risoluzione_short", "", 100,-0.5, 0.5);
  
        double t_true_short;
	float  t_rec_short;
	double res_short; //variabile per il calcolo della risoluzione

        datatree_short_rec->SetBranchAddress("time",&t_rec_short);
        datatree_short_rec->SetBranchAddress("Bd_TRUETAU",&t_true_short);
	   	
	//Risoluzione short
	  for (int i=0; i<datatree_short_rec->GetEntries(); i++) {
	  datatree_short_rec->GetEntry(i);	  
          t_true_short = t_true_short * 1000;
          
          res_short = t_rec_short - t_true_short;
          hmr_short->Fill(res_short);
         
         }


	//Plot risoluzione
       TCanvas *can2=new TCanvas("can2", "can2", 1600,1000);
       hmr_short->SetXTitle("t_rec - t_true [ps]");
       hmr_short->SetYTitle("Events");
       hmr_short->SetStats(0);
       hmr_short->GetYaxis()->SetTitleOffset(1.1);
       hmr_short->GetXaxis()->SetTitleOffset(1);
       hmr_short->GetYaxis()->SetTitleSize(0.048);
       hmr_short->GetXaxis()->SetTitleSize(0.048);
       hmr_short->Draw();
       can2->SaveAs("risoluzione_short.pdf");	
	...
	RooAddPdf model("model", "model", RooArgList(gauss1,gauss2, gauss3), RooArgList(fsig, fsig2));
        ...
        RooDataHist data_short("data", "data res", t, hmr_short);
	RooFitResult* fitResult = model.fitTo(data_short, RooFit::Save());
	...
	xframe->Draw();
	  ...
	  c->SaveAs("ris_fit_short.pdf");        
        
        double sigma_short = (fsig.getVal() * sigma1.getVal() + fsig2.getVal() * sigma2.getVal() + (1- fsig.getVal() - fsig2.getVal()) * sigma3.getVal())  ; 
        cout<<"sigma_short "<<sigma_short<<"\n";
