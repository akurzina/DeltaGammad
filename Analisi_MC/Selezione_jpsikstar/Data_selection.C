...
void Data_selection() {
  ...
  //Importazione tree originale
	TChain *datatree = new TChain("Bd2JpsiKstz/DecayTree");
	datatree->Add("B02JpsiKstar_2016_magDown_MC.root");
  ...
  //Variabili Stripping
	...
	//Creo nuovo tree
 	TFile newfile("B02JpsiKstar_2016_magDown_MC_selected.root", "recreate");
...
//datatree->GetEntries()
	...
	for(int i=0; i<datatree->GetEntries(); i++){
		...
		time = Bd_PVConst_ctau[0]/c/ps;
 		...
 		if(muminus_TRACK_CHI2NDOF > 4){continue;	}
 		if(muplus_TRACK_CHI2NDOF > 4){continue;	}
 		if(piminus_TRACK_CHI2NDOF > 4){continue;	}
		...
		if(Kstz_PT <= 1200){continue;	}
 		...
 		if((Bd_PVConst_ctau[0]/c/ps ) < (0.3) || (Bd_PVConst_ctau[0]/c/ps) > (14 ) ){continue;	}
